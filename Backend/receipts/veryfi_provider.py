...
def process(image_path):
    ...
    signature = _create_signature(config["client_secret"], {}, timestamp)
    ...
    return map_document_to_result(payload, image_path=image_path)


def map_document_to_result(payload, image_path):
    raw_text = (payload.get("ocr_text") or "").strip()
    if not raw_text:
        raw_text = extract_text_from_receipt(image_path)

    parsed_items = _parse_line_items(payload.get("line_items") or [])
    if not parsed_items:
        parsed_items = parse_receipt_text(raw_text)

    store_name = _extract_store_name(payload, raw_text)
    detected_total = _extract_total(payload)
...
def _extract_store_name(payload, raw_text):
...
def _parse_line_items(line_items):
    parsed_items = []
    ...
    for line_item in sorted(
        ...
        name = _extract_item_name(line_item)
        price_text = _extract_item_price(line_item)
        if not name or price_text is None:
            continue

        if _should_skip_line_item(line_item, name):
            continue
...
def _extract_item_name(line_item):
    ...
    for candidate in candidates:
        for fragment in _split_name_fragments(candidate):
...
def _split_name_fragments(value):
...
def _extract_item_price(line_item):
...
def _should_skip_line_item(line_item, name):
...
def _create_signature(secret, payload, timestamp):
