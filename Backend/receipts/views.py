...
def confirm_receipt(request, receipt_id):
    ...
    parsed_items_by_id = {
        item.id: item
        for item in receipt.parsed_items.all()
    }
    food_items = []
    ...
    with transaction.atomic():
        for item_data in items_data:
            ...
            parsed_item.standardized_name = item_data.get("standardized_name", parsed_item.standardized_name)
            parsed_item.quantity = int(item_data.get("quantity", parsed_item.quantity or 1))
            parsed_item.expiration_days = item_data.get("expiration_days", parsed_item.expiration_days)
